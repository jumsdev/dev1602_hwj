...
else if ($bk_payment == '신용카드') {
    switch($wzpconfig['pn_pg_service']) {
        default:
            include WZP_PLUGIN_PATH.'/kcp/pp_ax_hub.php';
...
if($tno) {
    if((int)$bk_price !== (int)$pg_price) {
        $cancel_msg = '결제금액 불일치';
        switch($wzpconfig['pn_pg_service']) {
            default:
                include WZP_PLUGIN_PATH.'/kcp/pp_ax_hub_cancel.php';
                break;
} 
