...
$goods      = $bk_subject . ($cnt_chk>1 ? ' 외'.($cnt_chk-1).'건' : '');

// PG 결제를 위한 코드
if ($wzpconfig['pn_pg_service']) { 
    @include_once(WZP_PLUGIN_PATH.'/'.$wzpconfig['pn_pg_service'].'/config.php');
    @include_once(WZP_PLUGIN_PATH.'/'.$wzpconfig['pn_pg_service'].'/orderform.1.php');
}
?>
...
<div class="st2-form">
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="tbl_type frm">
        ...
        <tr>
            ...
            <td> <br />
                 <h2>숯불 바베큐 이용을 원하시면 요청사항에 (ex. 숯불 바베큐 2set 사용)으로 글 남겨 주세요.</h2><br />
                 - 숯불 바베큐는 1SET 15,000원 (현장결제)
                <br /><br />
    ...
    <?php
    if ($wzpconfig['pn_pg_service']) { 
        @include_once(WZP_PLUGIN_PATH.'/'.$wzpconfig['pn_pg_service'].'/orderform.2.php');    
    }     
    ?>
